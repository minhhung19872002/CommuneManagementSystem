...
const TemporaryAbsence: React.FC = () => {
  const [data, setData] = useState<TempAbsence[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [persons, setPersons] = useState<Person[]>([]);
  const [form, setForm] = useState({ personId: '', startDate: '', endDate: '', reason: '', destination: '' });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const r = await tempAbsenceService.getAll(); setData(r.data); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { personService.getAll('Alive').then(r => setPersons(r.data)).catch(console.error); }, []);

  const handleCreate = async () => {
    try {
      await tempAbsenceService.create({ personId: Number(form.personId), startDate: form.startDate, endDate: form.endDate, reason: form.reason, destination: form.destination });
      toast_('Đăng ký tạm vắng thành công!'); setShowModal(false); setForm({ personId: '', startDate: '', endDate: '', reason: '', destination: '' }); load();
  ...
  const handleExtend = async (id: number) => {
    ...
    if (!newDate) return;
    try { await tempAbsenceService.extend(id, newDate); toast_('Gia hạn thành công!'); load(); } catch { toast_('Lỗi!', 'error'); }
  };

  const handleCancel = async (id: number) => {
    if (!confirm('Hủy đăng ký tạm vắng?')) return;
    try { await tempAbsenceService.delete(id); toast_('Hủy thành công!'); load(); } catch { toast_('Lỗi!', 'error'); }
  };

  const getBadge = (s: string) => {
    const map: Record<string, string> = { Active: 'badge-active', Returned: 'badge-returned', Cancelled: 'badge-cancelled' };
    const label: Record<string, string> = { Active: 'Đang tạm vắng', Returned: 'Đã trở về', Cancelled: 'Đã hủy' };
  ...
  return (
    <>
      <div className="topbar">
        <h1>✈️ Quản lý Tạm vắng</h1>
        <button className="btn btn-primary" onClick={() => setShowModal(true)}>+ Đăng ký tạm vắng</button>
      </div>
          ...
          <div className="card-body">
            {loading ? <div className="loading">Đang tải...</div> : (
              <table className="data-table">
                <thead><tr><th>#</th><th>Người</th><th>Từ ngày</th><th>Đến ngày</th><th>Nơi đến</th><th>Lý do</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
                <tbody>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><h3>Đăng ký tạm vắng</h3><button className="modal-close" onClick={() => setShowModal(false)}>×</button></div>
            <div className="modal-body">
              <div className="form-group"><label>Người tạm vắng</label>
              ...
              <div className="form-group"><label>Nơi đến</label><input value={form.destination} onChange={e => setForm(f => ({ ...f, destination: e.target.value }))} /></div>
