...
const frontendUrl = process.env.E2E_FRONTEND_READY_URL ?? `http://127.0.0.1:${frontendPort}/login`;
const apiBindUrl = process.env.E2E_API_BIND_URL ?? 'http://127.0.0.1:5068';
const apiReadyUrl = process.env.E2E_API_READY_URL ?? `${apiBindUrl}/openapi/v1.json`;
...
async function waitForUrl(url, label, timeoutMs = 180_000) {
  ...
  throw new Error(`[e2e] Timed out waiting for ${label}: ${url}`);
}
...
function spawnManaged(command, args, cwd, label, extraEnv = {}) {
  ...
  child.on('exit', code => {
    if (!shuttingDown && code !== 0) {
      console.error(`[e2e] ${label} exited unexpectedly with code ${code ?? 'null'}.`);
      void shutdown(1);
    }
  });

  children.push(child);
  console.log(`[e2e] Started ${label} (pid: ${child.pid ?? 'unknown'}).`);
}
...
async function ensureStack() {
  ...
  if (!apiReady) {
    ...
    console.log(`[e2e] Reusing backend at ${apiReadyUrl}.`);
  }

  if (!frontendReady) {
    spawnManaged(npmCommand, ['run', 'dev:e2e'], clientDir, 'frontend', { BROWSER: 'none' });
  } else {
    console.log(`[e2e] Reusing frontend at ${frontendUrl}.`);
  }
...
async function main() {
  ...
  await ensureStack();
  await new Promise(() => {
    // Keep the process alive until start-server-and-test terminates it.
  });
}
